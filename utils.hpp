...
RGBA ESPColorx = { 0, 0, 0, 255 };

namespace Settings {

	bool Aimbot = false, DrawFOV = false, Box = false, Skeleton = false, Head = false;
	bool CornerBox = false;
	bool FilledBox = false;
	bool Snapline = false;
	bool insta = false;
	bool noanim = false;
	bool airstruct = false;
		int distance, MaxSkeletonDrawDistance = 200, hitbox = 0, EspDistance = 200, AimbotDistance = 200;
		float AimbotFOVValue = 80.0f, AimbotSmoothingValue = 2.0f;
...
Vector3 ProjectWorldToScreen(Vector3 WorldLocation, Vector3 camrot)
{
	...
	Camera.x = Driver::read<float>(pid, chain699 + 0x7E8);
	...
	float zoom = Driver::read<float>(pid, chain699 + 0x580);
...
{
	uint64_t bonearray = Driver::read<uint64_t>(pid, + 0x4B0);
	if (!bonearray) bonearray = Driver::read<uint64_t>(pid, mesh + 0x4B0);
